...
function ReefForm({ initialValues, onSubmit, submitting, title }) {
  ...
  const update = (event) => { const { name, value, checked, type } = event.target; setValues((current) => ({ ...current, [name]: type === 'checkbox' ? checked : value })) }
  const submit = (event) => { event.preventDefault(); const payload = Object.fromEntries(Object.entries(values).map(([key, value]) => [key, numericFields.has(key) ? Number(value) : value])); payload.ghost_net_distance = values.ghost_net_distance === '' ? null : Number(values.ghost_net_distance); onSubmit(payload) }
  return <Card title={title}><form className="grid gap-4 md:grid-cols-2" onSubmit={submit}>{['reef_name', 'country', 'latitude', 'longitude', 'coral_health', 'sea_temperature', 'ghost_net_distance'].map((field) => <label className="block text-sm font-medium" key={field}>{field.replaceAll('_', ' ')}<input required={field !== 'ghost_net_distance'} name={field} value={values[field]} type={field === 'reef_name' || field === 'country' ? 'text' : 'number'} step="any" onChange={update} className="mt-1 block w-full rounded-md border border-slate-300 p-2" /></label>)}<label className="flex items-center gap-2 text-sm font-medium"><input name="bleaching_alert" type="checkbox" checked={values.bleaching_alert} onChange={update} />Bleaching alert</label><label className="flex items-center gap-2 text-sm font-medium"><input name="protected_area" type="checkbox" checked={values.protected_area} onChange={update} />Protected area</label><div className="md:col-span-2"><Button type="submit" disabled={submitting}>{submitting ? 'Saving…' : 'Save reef'}</Button></div></form></Card>
}

export default function ReefDetails() {
  const { id } = useParams(); const navigate = useNavigate(); const [reefs, setReefs] = useState([]); const [reef, setReef] = useState(null); const [loading, setLoading] = useState(true); const [submitting, setSubmitting] = useState(false); const [editing, setEditing] = useState(false)
  const load = async () => { setLoading(true); try { if (id) setReef((await reefService.getById(id)).data); else setReefs((await reefService.getAll()).data) } catch (error) { toast.error(messageFor(error)) } finally { setLoading(false) } }
  useEffect(() => { load() }, [id])
  const save = async (payload) => { setSubmitting(true); try { const response = id ? await reefService.update(id, payload) : await reefService.create(payload); toast.success(id ? 'Reef updated.' : 'Reef created.'); if (id) { setReef(response.data); setEditing(false) } else navigate(`/reef/${response.data.id}`) } catch (error) { toast.error(messageFor(error)) } finally { setSubmitting(false) } }
  const remove = async () => { if (!window.confirm(`Delete ${reef.reef_name}?`)) return; try { await reefService.remove(reef.id); toast.success('Reef deleted.'); navigate('/reefs') } catch (error) { toast.error(messageFor(error)) } }
